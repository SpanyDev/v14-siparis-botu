...
const { Events, PermissionFlagsBits, time } = require("discord.js");
        ...
        if (command.commandCooldown && command.commandCooldown > 0) {
            ...
            if (commandCooldowns.has(cooldownKey)) {
                ...
                if (currentTime < expirationTime) {
                    ...
                    const timeDiscordFormat = time(expirationTimestamp, "R");
                    return await interaction.reply({
                        content: `Please wait ${timeDiscordFormat} before reusing the \`${cmdName}\` command.`,
                        ephemeral: true
                    });
                }
            }
